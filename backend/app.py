...
CITY_COORDS_LOOKUP = {
    ...
    # High-risk-prone global regions
    "Dhaka": {"lat": 23.8103, "lon": 90.4125},       # Bangladesh - extreme flood risk
    "Manila": {"lat": 14.5995, "lon": 120.9842},      # Philippines - typhoon/cyclone hotspot
    "Port-au-Prince": {"lat": 18.5944, "lon": -72.3074},  # Haiti - earthquake + hurricane
    "Jakarta": {"lat": -6.2088, "lon": 106.8456},     # Indonesia - flood + earthquake
    "San Juan": {"lat": 18.4655, "lon": -66.1057},    # Puerto Rico - hurricane corridor
    "Kathmandu": {"lat": 27.7172, "lon": 85.3240},    # Nepal - seismic/earthquake zone
}
...
def get_fallback_weather(city):
    ...
    return {
        ...
        'wind_max': max(0, 8 + np.random.exponential(4)),
...
def report_sos():
    ...
    city_coords = CITY_COORDS_LOOKUP.get(city, {"lat": 19.0760, "lon": 72.8777})
    distance = calculate_distance(lat, lon, city_coords['lat'], city_coords['lon'])
    
    # Priority calculation (Severity constants)
    severity_map = {
        'Medical': 5,
        'Fire': 5,
        'Flood': 4,
    ...
    severity = severity_map.get(emergency_type, 1)
    
    # Higher score = Higher priority
    # Combined score of severity and proximity (closer = higher)
    priority_score = (severity * 10) / (distance + 0.1)
    
    request_id = len(sos_requests) + 1
    new_request = {
        'id': request_id,
        'name': name,
        'phone': phone,
    ...
    sos_requests.append(new_request)
    print(f"🚨 SOS RECEIVED: {name} in {city} ({emergency_type}) - Dist: {distance:.2f}km")
    
    return jsonify({
        "message": "SOS Reported Successfully. Help is on the way.",
...
def get_sos_list():
    ...
    sorted_sos = sorted(sos_requests, key=lambda x: x['priority_score'], reverse=True)
    return jsonify(sorted_sos)
